...
namespace RPG
{
    public class RPG : Mod
    {
        ...
        // We should keep the key press and actual start calls separate, as significantly different implementations could be required in the future
        //zzz this should be in MPlayer
        public void ActiveAbilityKeyPressed(MPlayer mplayer)
        {
            CheckForSendActiveAbilityPressed(mplayer);

            ActiveAbilityStart(mplayer);
        }

        //zzz this should be in MPlayer
        public void ActiveAbilityStart(MPlayer mplayer, bool overrideSpecial, int newSpecial)
        {
            if (overrideSpecial)
            {
                // This check should always be true, but we should confirm the packet was sent correctly and successfully
                if (mplayer.IsActiveAbilityKeyHeld())
                {
                    mplayer.special = newSpecial;
                }
                else
                {
                    //zzz Log error
